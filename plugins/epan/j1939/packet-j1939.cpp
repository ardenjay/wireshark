...
int dissect_J1939(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree,
				  void *data);
...
void proto_reg_handoff_j1939(void)
{
	...
	for (auto pgn = pgns.begin(); pgn != pgns.end(); ++pgn) {
		...
		std::unique_ptr<J1939Frame> frame =
			J1939Factory::getInstance().getJ1939Frame(*pgn);
		...
		for (auto spnNumber = spnNumbers.begin(); spnNumber != spnNumbers.end();
			 ++spnNumber) {
...
int dissect_J1939(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *)
{
	...
	// Get the buffer from the upper layer so that we can obtain the ID for the
	// Can Frame
	...
	if (tvb_reported_length(canIdTvb) >= 20) { // Not reassembled frames
		...
		canId = (leCanId >> 24) | ((leCanId >> 8) & 0xFF00) |
				((leCanId << 8) & 0xFF0000) | ((leCanId << 24) & 0xFF000000);
	...
	// Generate the frame from the frame factory. As arguments, we pass the ID,
	// the data and the length.
	std::unique_ptr<J1939Frame> frame =
		J1939Factory::getInstance().getJ1939Frame(canId, content, data_length);
	...
	ti = proto_tree_add_string(tree, hf_j1939_frame, canIdTvb, 1, 2,
							   frame->getName().c_str());
	...
	if (frame->isGenericFrame()) { // If the frame is a generic frame (this
								   // means that it contains several defined
								   // SPNs), call a proper function to handle it
		GenericFrame *genFrame = (GenericFrame *)(frame.get());
		dissect_generic_frame(tvb, j1939_tree, ti, genFrame);

	} else if (bamReassembler.toBeHandled(
				   *frame)) { // This frame is part of BAM protocol
		...
		if (!pinfo->fd->visited) { // Only treat the frames if it is the first
								   // time we see them!!!
			...
			case TP_CM_PGN: {
				...
				guint32 realCanId =
					(connFrame->getPriority() << J1939_PRIORITY_OFFSET) |
				...
				def_tvb = tvb_new_real_data(canIdTVBData, frag_data_len,
											frag_data_len);
		...
		bam_head = fragment_add_seq_check(
			&bam_reassembly_table, def_tvb, 0, pinfo, 0, NULL, sq,
		...
		next_tvb = process_reassembled_data(tvb, 0, pinfo, "Reassembled BAM",
											bam_head, &bam_frag_items,
											&update_col_info, j1939_tree);

		if (next_tvb) {
			call_dissector(j1939_handle,
						   tvb_new_subset_remaining(next_tvb, sizeof(guint32)),
						   pinfo, j1939_tree);
		}

	} else { // Another type of frame than the generic ones and the ones that
			 // belong to BAM protocol
	...
	for (auto iter = spnNumbers.begin(); iter != spnNumbers.end(); ++iter) {
		...
		case SPN::SPN_NUMERIC: {
			...
			ti = proto_tree_add_double_format(
				spn_tree, spnNumToHinfoId[*iter], tvb, spn->getOffset(),
		...
		case SPN::SPN_STATUS: {
			...
			ti = proto_tree_add_uint_bits_format_value(
				spn_tree, spnNumToHinfoId[*iter], tvb,
				(spn->getOffset() << 3) + 8 - spnStatus->getBitOffset() -
					spnStatus->getBitSize() /* + spnStatus->getBitOffset()*/,
		...
		case SPN::SPN_STRING: {
			...
			ti = proto_tree_add_item(spn_tree, spnNumToHinfoId[*iter], tvb,
									 spn->getOffset(),
...
void dissect_fms1_frame(tvbuff_t *tvb, proto_tree *j1939_tree, proto_item *,
						const FMS1Frame *fms1Frame)
{
	...
	for (int tts = firstTts; tts < firstTts + TTSS_PER_BLOCK; ++tts) {
		proto_tree_add_item(j1939_tree, ttsNumToHinfoId[tts], tvb,
							(((tts - 1) % TTSS_PER_BLOCK) + 1) / 2, 1, ENC_NA);
	}
}
					   ...
					   const DM1 *dm1Frame)
{
	...
	for (auto dtc = dtcs.begin(); dtc != dtcs.end(); ++dtc) {
		ti = proto_tree_add_item(j1939_tree, hf_j1939_dtc, tvb, offset,
								 DTC_SIZE, ENC_NA);
		dtc_tree = proto_item_add_subtree(ti, ett_j1939_dtc);

		ti = proto_tree_add_uint_bits_format_value(
			dtc_tree, hf_j1939_spn, tvb, (offset << 3), SPN_NUMBER_MAX_BITS,
			dtc->getSpn(), "%u", dtc->getSpn());

		ti = proto_tree_add_uint_bits_format_value(
			dtc_tree, hf_j1939_fmi, tvb, (offset << 3) + SPN_NUMBER_MAX_BITS, 5,
			dtc->getFmi(), "%u", dtc->getFmi());

		ti = proto_tree_add_uint_bits_format_value(
			dtc_tree, hf_j1939_oc, tvb,
			(offset << 3) + SPN_NUMBER_MAX_BITS + 5 + 1, 7, dtc->getOc(), "%u",
			dtc->getOc());
